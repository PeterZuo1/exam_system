...
public class KimiAiServiceImpl implements KimiAiService {
    ...
    public String callKimiAi(String prompt) throws InterruptedException {
        ...
        for (int i = 0; i < retryCount; i++){
            try {
                ...
                //jackson工具！ JsonObject JsonArray，处理返回的参数
                ...
                //成功继续执行，获取返回结果
                String context = resultJsonObject.getJSONArray("choices").getJSONObject(0).getJSONObject("message")
                        .getString("content");
                if (context.isEmpty()){
                    throw new RuntimeException("返回结果为空！请稍后再试！！");
                }
                return context;
            }
            catch (Exception e) {
                //失败次数
                log.error("调用KimiAi接口失败！正在第{}次重试...", i );
                if(i==retryCount){
    ...
    public List<QuestionImportVo> aiGenerateQuestions(AiGenerateRequestVo request) throws InterruptedException {
        //生成提示词
        String context = buildPrompt(request);
        log.info("调用KimiAi接口，生成提示词为：{}",context);
        //调用KimiAi接口，返回结果
        String prompt = callKimiAi(context);

        int start = prompt.indexOf("```json");
        int end = prompt.lastIndexOf("```");
        //判断结果格式是否正确
        if(start == -1 || end == -1)
            throw new RuntimeException("返回结果格式错误！请稍后再试！！");
        //截取 json ，+7是为了去掉 ```json
        prompt = prompt.substring(start + 7, end);
        ...
        for (int i = 0; i < questions.size(); i++) {
            ...
            //如果是选择 题
            if("CHOICE".equals(question.getString("type"))){
                List<QuestionImportVo.ChoiceImportDto> listChoice = new ArrayList<QuestionImportVo.ChoiceImportDto>();
                ...
                for (int j = 0; j < choices.size(); j++){
                    ...
                    choiceImportDto.setContent(choice.getString("content"));
                    choiceImportDto.setIsCorrect(choice.getBoolean("isCorrect"));
                    choiceImportDto.setSort(choice.getInteger("sort"));
                    listChoice.add(choiceImportDto);
                }
                questionImportVo.setChoices(listChoice);
            }
            //添加到list中
    ...
    public String buildSummaryPrompt(Integer totalScore, Integer maxScore, Integer questionCount, Integer correctCount) {
        ...
        StringBuilder prompt = new StringBuilder();
        prompt.append("你是一名资深的it教育专家，请为学生的考试表现提供专业的总评和学习建议：\n\n");
